<template>
  <div class="table">
    <template>
      <el-table :data="cate_req" style="width: 100%" row-key="id" :tree-props="{children: 'children'}">
...
export default {
  computed: {
    ...mapGetters({
      ...
      cate_one_req: "gengxin_cate_one_req",
    }),
  },
  methods: {
    ...mapActions({
      ...
      sc_cate_one_req: "sc_cate_one_req",
    }),
    fn1(id) {
      this.get_cate_one_req(id);
    },
    shanc(id) {
      this.sc_cate_one_req(id).then((res) => {
        if (res.data.code == 200) {
          this.get_cate_list(true);
        }
      });
    },
  },
